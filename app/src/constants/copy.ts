...
export const COPY = {
  ...
  main: {
    ...
    menuLabel: '메뉴',
    cartTitle: '손수레',
    capacityLabel: '손수레 적재량',
    speedLabel: '모으는 속도',
    nextItemLabel: '다음 고물까지',
    nextItemFull: '손수레가 가득 찼어요',
    rewardPickTitle: '고물을 주웠어요!',
    rewardIdleTitle: '그동안 이만큼 모였어요!',
    rewardCountFormat: (n: number, yeop: number) => `고물 ${n}개 · 약 ${yeop.toLocaleString()}냥`,
    rewardConfirm: '확인',
    ...
    boosterNoteFormat: (mult: number) => `1시간 동안 ${mult}배 빠르게`,
    ...
    yeopUnit: '냥',
    balanceLabel: '지금까지 모은 엽전',
    cashOutBtn: '토스 포인트로 받기',
    speedChipFormat: (pct: number) => `모으는 속도 ${pct.toLocaleString()}%`,
  ...
  exchange: {
    ...
    weeklyCapReached: '이번 주 교환 한도(5,000원)를 다 채웠어요',
    pendingText: '교환 처리 중...',
    summaryLabel: '이번 주 교환',
  ...
  menu: {
    ...
    resetConfirmMessage: '손수레·엽전·이번 주 교환액이 모두 0으로 돌아가요. 진행할까요?',
    resetConfirmYes: '초기화',
    resetConfirmNo: '취소',
  ...
  guide: {
    ...
    gomulLineFormat: (label: string, price: number) => `${label} — ${price}냥`,
    howTitle: '모으는 법',
    howIdle: '손수레에 고물이 시간마다 자동으로 쌓여요.',
    howActive: '광고로 줍거나 빠르게 모으면 비싼 고물이 잘 나와요.',
